    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (responseData.errors && Array.isArray(responseData.errors)) {
          // Display detailed field errors
          const errorMessages = responseData.errors
            .map((err: { field: string; message: string }) => `${err.field}: ${err.message}`)
            .join(', ')
          throw new Error(`Failed to create lead. ${errorMessages}`)
        } else if (responseData.error) {
          throw new Error(responseData.error)
        } else {
          throw new Error('Failed to create lead. Please check all required fields.')
        }
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'An error occurred while creating the lead. Please try again.'
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          />
          {errors.email && (
            <p className="mt-1 text-sm text-red-600">{errors.email.message || 'Valid email address is required'}</p>
          )}
        </div>
